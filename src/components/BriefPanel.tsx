...
function BriefPanelComponent({
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white">
      ...
      <div className="sticky top-0 z-10 bg-gradient-to-b from-white via-white to-white/95 backdrop-blur-sm border-b border-[#C7D2E1]/15">
        <div className="px-5 sm:px-7 pt-5 sm:pt-7 pb-4 sm:pb-5">
          ...
          <div className="flex items-start justify-between gap-4 mb-3">
            ...
            {onClose && (
              <button
                onClick={onClose}
                className="p-2 rounded-lg hover:bg-[#E8F2FF]/60 transition-all duration-200 flex-shrink-0 hover:shadow-sm"
          ...
          <div className="flex items-center gap-2.5 text-xs sm:text-sm text-[#64748B] flex-wrap">
            <span className="inline-flex items-center gap-1.5 px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-full bg-[#E8F2FF]/50 text-[#5AA6FF] font-semibold text-xs border border-[#5AA6FF]/25 hover:border-[#5AA6FF]/40 transition-all">
              {article.source}
            </span>
      ...
      <div className="flex-1 overflow-y-auto px-5 sm:px-7 py-5 sm:py-6">
        ...
        {article.leadQuote && (
          <div className="bg-gradient-to-br from-[#E8F2FF]/40 to-[#E8F2FF]/20 rounded-2xl border border-[#5AA6FF]/25 p-6 sm:p-7 hover:border-[#5AA6FF]/40 transition-all duration-300 hover:shadow-md">
            <div className="flex items-start gap-4">
              <svg className="w-7 h-7 text-[#5AA6FF] flex-shrink-0 mt-0.5 opacity-80" fill="currentColor" viewBox="0 0 24 24">
        ...
        {article.bullets5 && article.bullets5.length > 0 && (
          <section className="space-y-4 p-6 sm:p-7 bg-gradient-to-br from-[#E8F2FF]/30 to-[#E8F2FF]/10 rounded-2xl border border-[#5AA6FF]/20 hover:border-[#5AA6FF]/30 transition-all duration-300 hover:shadow-md" aria-labelledby="summary-heading">
        ...
        {article.whyItMatters && Object.keys(article.whyItMatters).length > 0 && (
          <div className="space-y-4 p-6 sm:p-7 bg-gradient-to-br from-[#E8F2FF]/40 to-[#E8F2FF]/20 rounded-2xl border border-[#5AA6FF]/20 hover:border-[#5AA6FF]/30 transition-all duration-300 hover:shadow-md">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#5AA6FF] to-[#8B7CFF] flex items-center justify-center">
            ...
            <div className="space-y-3">
              {Object.entries(article.whyItMatters).map(([role, insight]) => (
                <div key={role} className="bg-white/50 rounded-xl p-4 hover:bg-white/70 transition-all duration-300 border border-[#C7D2E1]/20 hover:border-[#5AA6FF]/30">

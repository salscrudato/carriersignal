  ...
  return (
    <div className="flex flex-col h-full w-full bg-white">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-[#F3F4F6]">
        <div className="px-4 sm:px-6 pt-4 sm:pt-5 pb-3 sm:pb-4">
          ...
          <div className="flex items-start justify-between gap-3 mb-2">
            <h2 className="text-lg sm:text-xl font-semibold text-[#111827] leading-snug line-clamp-2 flex-1 pr-2">{article.title}</h2>
            {onClose && (
              <button
                onClick={onClose}
                className="p-1.5 rounded-md hover:bg-[#F9FAFB] transition-all duration-200 flex-shrink-0 border border-transparent hover:border-[#E5E7EB]"
          ...
          <div className="flex items-center gap-2 text-xs text-[#9CA3AF] flex-wrap">
            <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-[#D1FAE5] text-[#10B981] font-medium text-xs border border-[#10B981]/20">
              {article.source}
            </span>
            <span className="text-[#D1D5DB]">•</span>
            <span className="text-[#9CA3AF] font-medium">{publishDate}</span>
            {timeAgo && (
              <>
                <span className="text-[#D1D5DB]">•</span>
                <span className="text-[#9CA3AF] font-medium">{timeAgo}</span>
              </>
            )}
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 sm:py-5">
        ...
        {article.leadQuote && (
          <div className="bg-[#F9FAFB] rounded-lg border border-[#E5E7EB] p-4 sm:p-5">
            <div className="flex items-start gap-3">
              <svg className="w-5 h-5 text-[#10B981] flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10h-9.983zm-14.017 0v-7.391c0-5.704 3.748-9.57 9-10.609l.996 2.151c-2.433.917-3.996 3.638-3.996 5.849h3.983v10h-9.983z" />
              </svg>
              <p className="text-sm italic text-[#111827] leading-relaxed">{article.leadQuote}</p>
            </div>
          </div>
        )}
        ...
        {article.bullets5 && article.bullets5.length > 0 && (
          <section className="space-y-3 p-4 sm:p-5 bg-[#F9FAFB] rounded-lg border border-[#E5E7EB]" aria-labelledby="summary-heading">
            <div className="flex items-center gap-2">
              ...
              <h3 id="summary-heading" className="font-semibold text-[#111827] text-sm">Summary</h3>
            </div>
            <ul className="space-y-2">
              {article.bullets5.map((bullet, idx) => (
                <li key={idx} className="flex gap-2.5 text-sm text-[#4B5563] leading-relaxed">
        ...
        {article.whyItMatters && Object.keys(article.whyItMatters).length > 0 && (
          <div className="space-y-3 p-4 sm:p-5 bg-[#F9FAFB] rounded-lg border border-[#E5E7EB]">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-full bg-[#10B981] flex items-center justify-center">
              ...
              <h3 className="font-semibold text-[#111827] text-sm">Why It Matters</h3>
            </div>
            <div className="space-y-2">
              {Object.entries(article.whyItMatters).map(([role, insight]) => (
                <div key={role} className="bg-white rounded-md p-3 border border-[#E5E7EB]">
                  <p className="text-xs font-semibold text-[#10B981] uppercase tracking-wide mb-1">{role}</p>
        ...
        {article.confidenceRationale && (
          <div className="p-4 bg-[#F9FAFB] rounded-lg border border-[#E5E7EB]">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-sm font-semibold text-[#111827]">Confidence</p>
            </div>
            <p className="text-sm text-[#4B5563] leading-relaxed">{article.confidenceRationale}</p>
          </div>
        )}
